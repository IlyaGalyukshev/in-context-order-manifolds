...
run python3 "$BAT" "${MP[@]}" --device-map auto --batch-size 8 \
...
step "STAGE 2 — E1 interior RSA (real vs coherence-twin)"
for FAM in s0_zib s1_loud s1_size; do for SC in card_mean readout; do for N in 9 12; do
  run python3 "$PRB" --acts "$A" "${PM[@]}" --families $FAM --condition shuffle --scheme $SC --n-items $N \
    --json "$S/rsa_${FAM}_${SC}_N${N}.json"
done; done; done
for FAM in s0_zib s1_loud s1_size; do for N in 9 12; do
  run python3 "$PRB" --acts "$A/probe" "${PM[@]}" --families $FAM --condition shuffle --scheme probe --n-items $N \
    --json "$S/rsa_${FAM}_probe_N${N}.json"
done; done
...
for DECL in none adjacency derived_summary list; do for N in 9 12; do
  run python3 "$PRB" --acts "$A" "${PM[@]}" --families s0_zib --condition shuffle --scheme card_mean \
    --declared $DECL --n-items $N --json "$S/e2_${DECL}_N${N}.json"
done; done
...
for FAM in s0_zib s1_loud s1_size; do for SUB in onehop multihop; do
  run python3 "$PRB" --acts "$A" "${PM[@]}" --families $FAM --condition shuffle --scheme card_mean --n-items 12 \
    --pair-subset $SUB --stimuli "$D/main/stimuli.jsonl" --json "$S/r8_${FAM}_${SUB}.json"
done; done
...
for RE in 1 2 3 4; do
  run python3 "$PRB" --acts "$A/hop" "${PM[@]}" --families s0_zib,s1_size --condition shuffle --scheme card_mean \
    --hop-reach $RE --n-items 16 --json "$S/decay_r${RE}.json"
done
...
for PT in order nonorder; do
  ...
  run python3 "$PRB" --acts "$A/e7q_$PT" "${PM[@]}" --families s0_zib --condition shuffle --scheme probe \
    --probe-type $PT --n-items 12 --json "$S/e7q_${PT}.json"
done
...
for F in 0.25 0.5 0.75 1.0; do
  run python3 "$PRB" --acts "$A/e8" "${PM[@]}" --families s0_zib --condition shuffle --scheme probe \
    --card-frac $F --n-items 12 --json "$S/e8_frac${F}.json"
done
