...
def main() -> None:
        ...
        for scheme in schemes:
            for layer in range(n_layers):
                ...
                for d, P in zip(data, pts):
                    for variant, X in (("raw", P), ("posproj", project_out(P, basis))):
                        t, diag = fit_coordinate(X)
                        q = quality_readout(t, d["ranks"], d["slots"])
                        rows.append({
    ...
    summ = (df.groupby(["model", "family", "condition", "scheme", "variant", "layer"])
            ["abs_q"].mean().reset_index())
    best = summ.loc[summ.groupby(["model", "family", "condition", "scheme", "variant"])
                    ["abs_q"].idxmax()]
    print("\n=== best layer by mean |q_content_partial| ===")
    for _, r in best.iterrows():
        print(f"{r['model']:14s} {r['family']:10s} {r['condition']:8s} {r['scheme']:10s} "
              f"{r['variant']:7s} L{int(r['layer']):>2d}  |q|={r['abs_q']:.3f}")
